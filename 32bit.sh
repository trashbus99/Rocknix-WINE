...
# ---------------------------
UNITY_OPT=0
yesno "Unity/Other Game Optimizations" "Would you like to apply additional optimal settings for Unity games (or similar titles)?\n\nThis will write extra BOX86 environment variable settings to the launcher script."
if [ $? -eq 0 ]; then
    UNITY_OPT=1
    msgbox "Additional Settings" "Extra BOX86 settings will be written to your launcher script."
fi
...
export BOX86_AVX=0\n\
export BOX86_MAXCPU=8\n\
export BOX86_UNITYPLAYER=1" "${LAUNCH_SCRIPT}"
fi
